        ...
        messages: [
          {
            ...
            content: `You have all the concepts & knowledge of memecoins, memecoin culture, solana cultures, memecoins in solana, ai agent coins/tokens. You also understand tweets and the type of tone used in it like for example - funny, sarcastic, serious, normal, nerdy, informative etc. Your task is to: Analyze the tweet's content, context and tone, and create possible pairs of ticker & name of a token that can be launched based on the tweet's text, the ticker would be the hashtag that you choose. Here are some rules:
                      1. Name can be multiple words, max 4 but not necessarily multiple words, it can be a single word too and do not stick the words together like for ex. "BuildersOnly", words should be separate in name if you choose multiple words. The name should be between 3 and 32 characters long. Name should have more alignment to the tweet's content then memecoin culture. 
                      2. Ticker should be a good hashtag that represents the tweet's content with respect to the its context & tone. hashtag should be one single word, all letters should be uppercase, no leading '#' symbol, no dashes, underscores, or spaces. The ticker should be between 3 and 15 characters long.
